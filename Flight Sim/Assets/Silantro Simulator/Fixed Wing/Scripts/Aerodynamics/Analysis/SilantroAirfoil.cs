		...
		if(lift != null && lift.Count > 1)
		{
			for (int i = 0; i < lift.Count-1;i++)
			{
				...
				if(currentAlpha > -40f && currentAlpha < 40)
				{
					...
					if (currentLiftLimit > lowerLiftLimit && currentLiftLimit < 0) { lowerLiftPoint = currentLiftLimit; lowerAnglePoint = currentAlpha;
...
public class SilantroAirfoilEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Space(2f);
		EditorGUILayout.LabelField("Clα", foil.centerLiftSlope.ToString("0.000"));
