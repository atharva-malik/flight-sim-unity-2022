...
{
    public class MyLandingGear : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.G) && m_State == GearState.Lowered && m_Plane.Altitude < lowerAtAltitude && m_Rigidbody.velocity.y > 0)
            {
                m_State = GearState.Raised;
            }

            if (Input.GetKeyDown(KeyCode.G) && m_State == GearState.Raised && m_Plane.Altitude < lowerAtAltitude && m_Rigidbody.velocity.y < 0)
            {
